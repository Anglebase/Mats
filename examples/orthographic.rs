...
impl Drawable for Canvas {
    fn draw(&mut self, window: &Window, display: &Display<WindowSurface>) {
        ...
        let program = glium::Program::from_source(
            display,
            include_str!("./res/box.vert"),
            include_str!("./res/box.frag"),
            None,
        )
        .unwrap();
        ...
        let time = SystemTime::now()
            ...
            .as_secs_f32()
            * 30.0;
        let transform = rotate3d::<f32>([1.0, 1.0, 0.0].into(), radian(time));
        let view = translate3d([0.0, 0.0, -5.0].into());
        let aspect = window.inner_size().width as f32 / window.inner_size().height as f32;
        let k = 2.0;
        let pre = orthographic((-k * aspect, k, k * aspect, -k), 0.1, 100.0);
